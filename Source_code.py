...
def select_quality(mode=None,path=None):
    if path is not None:
        ...
        Label(frame4,text='Selected Path',border=5,relief='solid',background='light blue',foreground='black',font=('Imprint MT Shadow',10)).grid(row=0,column=0,ipadx=5,columnspan=2,sticky='e')
        Label(frame4,foreground='white',text=path,border=5,relief='solid',font=('Imprint MT Shadow',10)).grid(row=0,column=2,columnspan=5,sticky='w')
        
    # Display video title
    Label(frame4,text=f'Video Founded\n{title[0:min(len(title),68)]}....',border=5,relief='solid',foreground='white',font=('Imprint MT Shadow',10)).grid(row=1,column=0,columnspan=9,ipady=2,sticky='w')

    # Display select quality label
    Label(frame4,text='Select Quality',border=10,relief='solid',background='light blue',foreground='black',font=('Imprint MT Shadow',10)).grid(row=2,column=1,columnspan=2)
    
    # Create a combobox to select quality
    selected_quality=Combobox(frame4,values=quality,state='readonly',bootstyle='danger',width=38)
    selected_quality.grid(row=2,column=3,columnspan=4,ipady=1)
    selected_quality.current(0)
    
    # Create a download button
    Button(frame4,text='Download',command=lambda:Download_file(selected_quality.current(),mode,path)).grid(row=2,column=7,sticky='e')
...
if __name__ == '__main__':
    ...
    Button(frame1,text='Downlode Playlist',command=lambda:print('Yes'),bootstyle='warning-outline').grid(row=0,column=3,ipady=3)
